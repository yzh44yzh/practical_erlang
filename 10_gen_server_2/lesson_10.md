...
Этот callback вызвается при горячем обновлении кода. Такое обновление
...
рассмотрим самый простой.  В init частично инициализируется State, и
...
Если не регистрируется, то таких объектов может быть много, и нужно
...
время не приходит ответ, то поток крашится. Обычно этим и проявляется
такой deadlock.  Но если вы зачем-то заменили дефолтный таймаут на
infinity, то поток в таком состоянии будет висеть бесконечно.
В какой-то момент его очередь сообщений исчерпает всю доступную
оперативную память, и нода упадет.

Выводы:
- помните по такой deadlock
