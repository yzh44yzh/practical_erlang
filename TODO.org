...
- разобрать на части, вернуть мапу
  {protocol => "https", domain => [com, youtube, www], path = [user, ErlangSolutions, playlists], get_params = #{<<"list">> => <<"PL">>}}
- выделить домен, проверить, что он входит в список доверенных доменов
- из path выделить год-месяц-день, преобразовать в int, вернуть дату

для каждой ссылки вернуть: разобранный урл, доверенный или нет домен, дата если есть
