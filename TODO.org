* TODO exercise 15. Обработка ошибок на разных уровнях
  хорошо бы, чтобы в задании было 2 варинта обработки ошибок:
  let it crash и try..catch

  Для try..catch можно дать задание по валидации входящих данных
  куча условий по валидации, при нарушении любого бросаем исключение
  так избегаем глубокой вложенности
  (без ковбоя и без реального хттп запроса)

  Однако, именно это описано в теории урока.
  Так что для практики надо бы что-то другое.
