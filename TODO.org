* TODO поправить упражнение
1) в условии get_rooms должна вернуть Rooms, а в тестах ожидается {ok, Rooms}
2) в условии get_messages должна вернуть Messages, а в тестах ожидается {ok, Messages}
3) в условии get_users должна вернуть Users, а в тестах ожидается {ok, Users}
4) Порядок пользователей в get_users не описан в условии, а тест ожидает обратный порядок входа
(что обусловленно реализацией через list), хотя интуитивно напрашивается прямой порядок

* TODO курсовая работа
- ff_game
  - list_zipper:find + tests
    - вероятно понадобится возвращать ошибку, когда дошел до края
